...
def login(request, db):
    ...
    user_profile = db.query(models.Profile).filter(models.Profile.username == request.username.lower())
    try:
        if (user.first() and Hash.verify_password(request.password, user.first().password)) :
            ...
            access_token = create_access_token(
                data={"sub": request.username.lower()}, expires_delta=access_token_expires
            )
...
def register(request,db):
    ...
    if user:
        user_profile = db.query(models.Profile).filter(models.Profile.username == request.username.lower()).first()
    ...
    if user and (not user_profile.is_active or user_profile.is_active):
        ...
        try:
            ...
            db.add(create_user)
            db.commit()
            profile_created = profile.create_profile(request.username.lower(),request.first_name,request.last_name,db)
            if profile_created:
               return True
        except:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail='Something went wrong')

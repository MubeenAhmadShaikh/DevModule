...
def add_review(proj_id, request, db, current_user):
    ...
    if current_user.id == projectObj.owner.id:
        raise HTTPException(
            ...
            detail="You Cannot add review to your own project",
        )
        ...
        for review in curr_project_review:
            if review.owner is current_user:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="You have already added your review",
                )
        else:
...
def update_vote_count(proj_id, db):
    ...
    vote_update = {"vote_total": projectObj.first().vote_total + 1}
...
def update_vote_ratio(proj_id, vote_ratio, db):
    projectObj = db.query(models.Project).filter(models.Project.id == proj_id)
    vote_ratio_update = {"vote_ratio": round(vote_ratio, 2)}
...
def get_positive_feedback(project_id, db):
    ...
    reviews = (
        db.query(models.Review).filter(models.Review.project_id == project_id).all()
    )
    if len(reviews) != 0:
        up_votes = 0

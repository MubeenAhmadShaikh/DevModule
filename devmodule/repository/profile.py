...
def search_profiles(
    query: str, page_start, page_end, db: Session = Depends(database.get_db)
):
    all_profiles = (
    ...
    if all_profiles:
        ...
        short_intro_profiles = (
            db.query(models.Profile)
            .filter(models.Profile.short_intro.contains(query))
            .all()
        )
        short_intro_active_profiles = all_active_profiles(short_intro_profiles)
        skill_filter = (
            db.query(models.Skill).filter(models.Skill.name.contains(query)).all()
        )
...
def view_single_profile(id, db):
    ...
    if not user_profile:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="No such profile exist"
        )
    return user_profile
...
def update_profile(
    ...
    try:
        ...
        f.close()
        page = "profile"
        profile_img_id = driveDB.upload_file(
            profile_image.filename,
            os.path.realpath(os.curdir)
            + "/temp/profile_images/"
            + profile_image.filename,
        ...
        profileObj = (
            db.query(models.Profile)
            .filter(models.Profile.id == current_user.id)
            .first()
        )
        ...
        raise HTTPException(
            ...
            detail="Unable to update your project",
        )
...
def deactivate_user(
    ...
    is_active1 = {"is_active": False}
    user_profile = db.query(models.Profile).filter(
        models.Profile.user_id == current_user.id
    )
    if not user_profile.first():
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="No such user exist"
        )
